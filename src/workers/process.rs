use crate::{
    core::{
        ActivityLogEntry, AppState,
        identify::{
            candidate_source_list, dedupe_candidates, identify, sort_candidates_for_decision,
        },
    },
    db::cache::{self as fingerprint_cache},
    domain::{
        audio,
        matcher::{artist_similarity, text_similarity, title_similarity},
    },
    media::fingerprint,
    workers::{
        persist::{
            persist_analysis_identity, persist_corrupt, persist_duplicate_members, persist_failed,
            persist_review, persist_unmatched,
        },
        scan::{
            FileJob, FingerprintEvidence, PersistJob, PipelineLimits, ProcessOutcome,
...
pub(crate) async fn process(
    ...
    match crate::media::integrity::check(&state.pool, path).await {
        Ok(crate::media::integrity::Integrity::Healthy) => {
            state
                .log_entry(
    ...
    let smart_decision = crate::workers::approve::rank(
