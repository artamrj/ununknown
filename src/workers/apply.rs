use crate::{
    ...
    db::{
        cache as fingerprint_cache,
        queries::{CandidateRow, TRACK_FIELDS, Track, candidates_by_track, selected_for_tracks},
    },
    media::{fingerprint, replaygain, tags},
    providers::Candidate,
    types::{TrackId, WorkflowPhase},
    workers::dedup::{self, RecordingEvidence},
};
...
pub(crate) async fn prepare_apply(s: &Arc<AppState>) -> Result<PreparedApply> {
    ...
    for group in dedup::group_recordings(&evidence) {
...
async fn recording_evidence(
    ...
    let content_key = if track.content_fingerprint.is_some() {
        ...
        dedup::fingerprint_key(&value.fingerprint)
    } else {
        Some(dedup::hash_key(&dedup::sha256_cached(pool, path).await?))
    };
...
async fn existing_file_has_expected_artwork(
    ...
    let matches =
        tokio::task::spawn_blocking(move || match crate::media::tags::read_artwork(&existing) {
...
pub async fn apply(
    ...
    for (i, item) in items.into_iter().enumerate() {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            let sanitized = tags::write_resilient(
                &write_target,
                &candidate,
                Some(artwork.clone()),
                replay_gain,
            )?;
            tags::verify_written_metadata(&write_target, &candidate)?;
            tags::verify_embedded_artwork(&write_target, &artwork)?;
...
pub(crate) async fn resolve_artwork(
    ...
    match crate::workers::artwork::resolve_for_write(&state.pool, &state.client, candidate).await {
        Ok(bytes) => {
            state
                .log_entry(
...
mod tests {
    ...
    async fn existing_output_with_different_artwork_is_not_reused() {
        ...
        let artwork_a = crate::media::tags::test_artwork_png();
        let artwork_b = other_artwork_png();
        crate::media::tags::write(&existing, &candidate, Some(artwork_a.clone()), None).unwrap();
        crate::media::tags::write(&temporary, &candidate, Some(artwork_b.clone()), None).unwrap();
        ...
        assert!(!temporary.exists());
        assert_eq!(
            crate::media::tags::read_artwork(published.path()).unwrap(),
            Some(artwork_b)
        );
        assert_eq!(
            crate::media::tags::read_artwork(&existing).unwrap(),
            Some(artwork_a)
        );
    }
