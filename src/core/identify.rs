use crate::{
    core::{
        ...
        state::{ActivityLogEntry, AppState},
    },
    domain::{
        audio,
    ...
    workers::{
        persist::provider_display_name,
        scan::{FingerprintEvidence, PipelineLimits},
    },
};
use anyhow::Result;
...
pub(crate) fn unique_exact_catalog_match(
    ...
    let current_album = current
        ...
        .as_deref()
        .filter(|album| !album.trim().is_empty() && !album.trim().starts_with('@'));
    let best_album_similarity =
        current_album
            .zip(best.album.as_deref())
            .map(|(album, candidate_album)| {
                crate::domain::matcher::text_similarity(album, candidate_album)
            });
