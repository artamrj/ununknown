...
function ManualEditor({ track, candidate, onSaved }: { track: Track; candidate?: Candidate; onSaved: () => Promise<void> }) {
  ...
  return <section className="manual-editor" aria-labelledby="manual-title">
    ...
    <div className="source-resolver"><label><span>Use a track link</span><input value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="Shazam, Spotify, SoundCloud, Audiomack, Navahang, Genius, Radio Javan, or YouTube URL" /></label><button className="compact-button" disabled={!sourceUrl.trim() || resolving} onClick={() => void resolveSource()}>{resolving ? "Loading…" : "Import"}</button></div>
...
function ProviderStatus({ sources }: { sources: Record<string, boolean> }) {
  ...
  return <section className="provider-settings"><div className="section-heading"><div><h3>Recognition tools</h3><p>{available} sources and local tools available</p></div><span className="health-dot">Ready</span></div><div className="provider-pills">{["musicbrainz", "deezer", "itunes", "radiojavan", "navahang", "audiomack", "genius", "spotify", "acoustid", "shazam", "songrec", "ffmpeg"].map((source) => <span className={sources[source] ? "active" : "inactive"} key={source}><i />{providerName(source)}</span>)}</div>{!sources.songrec && <p className="tool-warning"><Icon name="alert" size={15} />Install SongRec to recognize difficult files through Shazam. Catalog matching still works without it.</p>}{!sources.ffmpeg && <p className="tool-warning"><Icon name="alert" size={15} />Install FFmpeg for audio integrity checks and ReplayGain. Metadata cleaning still works.</p>}</section>;
}
