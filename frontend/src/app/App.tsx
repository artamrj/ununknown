...
function ProviderStatus({ sources }: { sources: Record<string, boolean> }) {
  ...
  return <section className="provider-settings"><div className="section-heading"><div><h3>Recognition tools</h3><p>{available} sources and local tools available</p></div><span className="health-dot">Ready</span></div><div className="provider-pills">{["musicbrainz", "deezer", "itunes", "radiojavan", "audiomack", "genius", "spotify", "acoustid", "songrec", "ffmpeg"].map((source) => <span className={sources[source] ? "active" : "inactive"} key={source}><i />{providerName(source)}</span>)}</div>{!sources.songrec && <p className="tool-warning"><Icon name="alert" size={15} />Install SongRec to recognize difficult files through Shazam. Catalog matching still works without it.</p>}{!sources.ffmpeg && <p className="tool-warning"><Icon name="alert" size={15} />Install FFmpeg for audio integrity checks and ReplayGain. Metadata cleaning still works.</p>}</section>;
}
